...
from SSL.losses import loss_cot, loss_diff, loss_sup, JensenShanon, js_from_softmax
...
S = nn.Softmax(dim=1)
loss_cot(a, b), js_from_softmax(S(a), S(b))
...
group_h.add_argument("--fusion_method", default="arithmetic_mean")
...
def train(epoch):
    ...
    for batch, (S1, S2, U) in enumerate(train_loader):
        ...
        with autocast():
            ...
            student_u1 = softmax_fn(logits_u1)
            student_u2 = softmax_fn(logits_u2)

            # logits_student_u = logits_u1
            l_teacher = consistency_cost(student_u1, softmax_fn(logits_tu))
            l_teacher += consistency_cost(student_u2, softmax_fn(logits_tu))
